...
pub fn Noul(comptime Instructions: type, comptime Criteria: type) type {
    ...
    return struct {
        ...
        pub const Answer = answer_types.NoulAnswer;
...
pub fn Choice(comptime Option: type, comptime Instructions: type, comptime Descriptions: type) type {
    comptime answer_types.checkOptionEnum(Option);
    ...
    return struct {
        ...
        /// The answer the API returns for this question.
        pub const Answer = answer_types.ChoiceAnswer(Option);
...
pub fn Score(comptime level_count: usize, comptime Instructions: type, comptime Levels: type) type {
    comptime answer_types.checkLevelCount(level_count);
    ...
    return struct {
        ...
        /// The answer the API returns for this question.
        pub const Answer = answer_types.ScoreAnswer(level_count);
...
test "score encodes levels as an array and normalizes string levels" {
    ...
    try std.testing.expect(@TypeOf(q).Answer == answer_types.ScoreAnswer(3));
    const out = try std.json.Stringify.valueAlloc(gpa, q, .{});
    defer gpa.free(out);
    try std.testing.expectEqualStrings(
...
test "answers mirror question ids and types" {
    ...
    try std.testing.expect(@FieldType(A, "is_urgent") == answer_types.NoulAnswer);
    try std.testing.expect(@FieldType(A, "department") == answer_types.ChoiceAnswer(Team));
    try std.testing.expect(@FieldType(A, "frustration") == answer_types.ScoreAnswer(3));
